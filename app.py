...
def init_db():
    if not DATABASE_URL:
        print("DATABASE_URL ausente. Pulo na inicialização do DB.")
        return
    ...
    try:
        ...
        # Impede mesmo horário de ser bloqueado duas vezes
        con.execute(
            """
            CREATE UNIQUE INDEX IF NOT EXISTS unique_blocked_slot
            ON blocked_slots (blocked_date, blocked_time)
            """
        )
...
def appointment_conflict(con, day, time, duration, ignore_id=None):
    appointments = con.execute(
        ...
        FROM appointments
        WHERE appointment_date = %s AND status <> 'Cancelado'
        """,
        (day,)
    ).fetchall()

    new_start = datetime.strptime(f"{day} {time}", "%Y-%m-%d %H:%M")
    new_end = new_start + timedelta(minutes=duration)

    for appointment in appointments:
        if ignore_id is not None and appointment["id"] == ignore_id:
            continue

        existing_start = datetime.strptime(
            f"{day} {appointment['appointment_time']}", "%Y-%m-%d %H:%M"
        )
        existing_duration = service_duration(appointment["service"])
...
def blocked_slot_conflict(con, day, start_time, duration):
    blocked = con.execute(
        """
        SELECT blocked_time
        FROM blocked_slots
    ...
    new_start = datetime.strptime(f"{day} {start_time}", "%Y-%m-%d %H:%M")
    new_end = new_start + timedelta(minutes=duration)

    for item in blocked:
        blocked_start = datetime.strptime(
            f"{day} {item['blocked_time']}", "%Y-%m-%d %H:%M"
        )
        blocked_end = blocked_start + timedelta(minutes=30)
...
def slots_for(day, service=None):
    ...
    if d.weekday() <= 4:
        if d.weekday() == 2:  # Quarta-feira
            periods = [(7, 30, 11, 0), (13, 30, 15, 0)]
        else:  # Segunda, Terça, Quinta, Sexta
            periods = [(7, 30, 11, 0), (13, 30, 20, 0)]
    else:  # Sábado
        periods = [(9, 0, 16, 0)]

    for start_hour, start_minute, end_hour, end_minute in periods:
        period_start = datetime.combine(d, datetime.min.time()).replace(
            hour=start_hour, minute=start_minute
        )
        period_end = datetime.combine(d, datetime.min.time()).replace(
        ...
        while current <= period_end:
            appointment_end = current + timedelta(minutes=duration)
...
def horarios():
    ...
    available = []
    blocked_times = {item["blocked_time"] for item in blocked}

    for slot in slots_for(day, service):
        slot_start = datetime.strptime(f"{day} {slot}", "%Y-%m-%d %H:%M")
...
def agendar():
    ...
    try:
        ...
        if blocked_slot_conflict(con, day, time, duration):
            flash("Esse horário está bloqueado.", "error")
            return redirect(url_for("index"))

        if appointment_conflict(con, day, time, duration):
...
with app.app_context():
    try:
        ...
        print(f"Falha ao rodar init_db na inicialização: {e}")
